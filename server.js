...
userSessions.set(user.username, null); // 暂时设置为null，等待Socket连接后更新
      ...
      try {
        ...
        if (employees.length > 0) {
          ...
          // 如果员工有关联的角色ID，直接通过角色ID获取权限
          if (employee.roleId) {
            console.log(`员工 ${employee.name} 有关联角色ID: ${employee.roleId}`);
            ...
            if (rolePerms.length > 0) {
              ...
              console.log(`从角色ID ${employee.roleId} 获取权限:`, permissions.map(p => p.name));
            }
          }

          // 如果通过roleId没找到权限，尝试通过部门职位匹配角色
        ...
        console.log(`用户 ${user.username} 最终权限:`, permissions.map(p => p.name));
      } catch (permError) {
