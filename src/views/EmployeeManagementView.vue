      ...
      <div class="content-wrapper">
        ...
        <div class="employee-section">
          ...
          <div class="section-header">
            ...
            <el-button type="primary" @click="openAddEmployeeDialog" class="add-btn" v-if="hasPerm('btn_add')">
          ...
          <div class="excel-actions">
            ...
            <el-button type="success" @click="exportEmployees" :loading="exportLoading" v-if="hasPerm('btn_export')">
    ...
    >
      <div style="padding: 10px 0;">
        <p style="margin: 0 0 12px; color: #555; font-size: 14px;">
          员工「<strong>{{ initPwdEmployeeName }}</strong>」的初始密码如下，请妥善告知员工：
        </p>
        ...
        <p style="margin: 0; color: #f56c6c; font-size: 13px;">
          请将此密码告知员工，员工登录后请在【修改密码】中更改为自己的密码。
        </p>
      </div>
      <template #footer>
...
const copyInitPwd = async () => {
  try {
    ...
    // 兼容不支持 clipboard API 的环境，降级为选中文本
    try {
      ...
      if (valid) {
        ...
        try {
          ...
          if (response.success) {
            ...
            // 若后端自动生成了初始密码，用专用对话框（纯文本+复制）展示
            if (response.initialPassword) {
              initPwdValue.value = String(response.initialPassword)
              initPwdEmployeeName.value = employeeForm.value.name || ''
...
const deleteEmployee = (name: string) => {
  ...
  try {
    ...
    ElMessageBox.prompt(
      `此操作将永久删除员工"${name}"（含其登录账号），请输入您的当前登录密码以确认：`,
      '高危操作确认',
      {
        inputType: 'password',
      ...
      try {
        ...
        // 调用API删除员工（携带 adminPassword 做二次验证）
        const response = await apiDeleteEmployee(name, value);
